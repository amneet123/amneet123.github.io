...
if (!class_exists('RadiusCheck')) {
	class RadiusCheck {
		...
		function __construct($Latitude, $Longitude, $Miles) {
			global $maxLat,$minLat,$maxLong,$minLong,$DecmaxLat,$DecminLat,$DecmaxLong,$DecminLong;
			...
			$DecmaxLat = $Latitude + $Miles / $EQUATOR_LAT_MILE;
			$DecminLat = $Latitude - ($maxLat - $Latitude);
			$DecmaxLong = $Longitude + $Miles / (cos($minLat * M_PI / 180) * $EQUATOR_LAT_MILE);
			$DecminLong = $Longitude - ($maxLong - $Longitude);
		...
		function DecMaxLatitude() {
			return round( $GLOBALS["DecmaxLat"] );
		}
		function DecMinLatitude() {
			return round( $GLOBALS["DecminLat"] );
		}
		function DecMaxLongitude() {
			return round( $GLOBALS["DecmaxLong"] );
		}
		function DecMinLongitude() {
			return round( $GLOBALS["DecminLong"] );
		} 
	}
}
/*
